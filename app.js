...
async function renderAdmin() {
  ...
  try {
    ...
    const { data: profiles, error } = await _sb.from('user_profiles').select('*').order('created_at', { ascending: false });
    ...
    function fmtLastSeen(ts) {
      if (!ts) return 'Never';
      const d = new Date(ts), diffH = Math.floor((Date.now() - d) / 3600000), diffD = Math.floor(diffH / 24);
    ...
    users.forEach(function(u) {
      const isYou    = u.email === (_currentSession && _currentSession.user && _currentSession.user.email);
      const lastSeen = fmtLastSeen(u.last_sign_in_at);
      const joined   = fmtJoined(u.created_at);
      const initial  = (u.email || '?')[0].toUpperCase();
      const dotColor = (lastSeen === 'Just now' || lastSeen.indexOf('h ago') > -1) ? 'var(--green)' : lastSeen === 'Never' ? 'var(--text3)' : 'var(--blue)';
      const avatarBg = isYou ? 'var(--phoenix)' : 'var(--surface3)';
      const avatarColor = isYou ? 'var(--phoenix-dark)' : 'var(--text2)';
      rows += '<tr style="border-bottom:1px solid var(--border)">'
        + '<td style="padding:14px 20px">'
          + '<div style="display:flex;align-items:center;gap:12px">'
            + '<div style="width:36px;height:36px;border-radius:50%;background:' + avatarBg + ';display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:' + avatarColor + ';flex-shrink:0">' + initial + '</div>'
            + '<div>'
    ...
    const email = _currentSession && _currentSession.user ? _currentSession.user.email : '—';
    if (listEl) listEl.innerHTML = '<div style="padding:20px 24px">'
      ...
      + '<div style="width:36px;height:36px;border-radius:50%;background:var(--phoenix);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:var(--phoenix-dark)">' + email[0].toUpperCase() + '</div>'
      + '<div><div style="font-weight:600;color:var(--text)">' + esc(email) + '</div>'
      + '<div style="font-size:11px;color:var(--text3);margin-top:2px">Full admin access · To manage other users, visit Supabase dashboard</div></div>'
      + '</div></div>';
...
async function adminRemoveUser(id, email) {
  ...
  if (!confirmed) return;
  showToast('To remove: Supabase dashboard → Authentication → Users → delete');
}
