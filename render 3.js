...
function renderPerformance() {
  ...
  // Include jobs where last history date OR poDate falls in the period
  // (imported jobs without chatter only have poDate as their date anchor)
  const donePeriod = jobs.filter(j => {
    if (j.status !== 'Job Done') return false;
    const last = j.history?.[j.history.length - 1];
    const lastDate = last?.date || '';
    return lastDate >= cutoffStr || (j.poDate || '') >= cutoffStr;
  });
...
function buildPrintReport() {
  ...
  <div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:10px;color:#1e2024;-webkit-print-color-adjust:exact;print-color-adjust:exact">
    ...
    <div style="display:grid;grid-template-columns:1.15fr 1.05fr 0.8fr;gap:10px;align-items:start">
      ...
      <!-- ── COL 1: OVERDUE ── -->
      <div>
        ${sHead(`Overdue Jobs <span style="font-weight:500">— 21d+ open</span>`,'#dc2626',urgent.length ? urgent.length+' jobs' : '')}
        ${urgent.length ? `
        ...
        ` : `
        <div style="display:flex;align-items:center;gap:6px;padding:12px 8px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;margin-top:4px">
          <span style="font-size:14px">✓</span>
          <span style="font-weight:700;color:#16a34a;font-size:9px">No overdue jobs</span>
        </div>`}
        ...
        ` : `
        <div style="display:flex;align-items:center;gap:6px;padding:8px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;margin-top:4px">
          <span style="font-size:12px">✓</span>
          <span style="font-weight:700;color:#16a34a;font-size:9px">No revisiting jobs</span>
        </div>`}
      </div>

      <!-- ── COL 2: PARTS + SERVICE CO. ── -->
      <div>
        ...
        ` : `
        <div style="display:flex;align-items:center;gap:6px;padding:10px 8px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;margin-top:4px">
          <span style="font-size:12px">✓</span>
          <span style="font-weight:700;color:#16a34a;font-size:9px">No parts delays</span>
        </div>`}
        ...
        <table style="width:100%;border-collapse:collapse">
          ...
          <tbody>${supplierStats.slice(0,8).map(x=>`<tr style="border-bottom:1px solid #f3f4f6">
            ...
            ${td(x.critical>0?`<span style="color:#dc2626;font-weight:700;font-size:9px">${x.critical}</span>`:`<span style="color:#d1d5db;font-size:9px">0</span>`,'text-align:center')}
            ${td(`<span style="font-size:8.5px;color:${x.avg>21?'#dc2626':x.avg>14?'#d97706':'#374151'};font-weight:600">${x.avg}d</span>`,'text-align:right')}
          </tr>`).join('')}
          </tbody>
        </table>
      </div>
      ...
      <div>
        ...
        ${stageAvgs.length ? stageAvgs.map(d=>{
        ...
        ${sHead('Key Stats')}
        <div style="background:#f8f9fa;border:1px solid #e5e7eb;border-radius:7px;padding:8px 9px;font-size:8px">
          ...
          <div style="margin-top:6px;padding-top:6px;border-top:1px solid #e5e7eb">
            <div style="font-size:7px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#dc2626;margin-bottom:2px">Longest Open Job</div>
            <div style="font-size:8px;font-weight:600;color:#1e2024">${esc(longestJob.ref||longestJob.po)}</div>
            <div style="font-size:7.5px;color:#6b7280">${esc(longestJob.supplier)} · <strong style="color:#dc2626">${longestDays}d</strong></div>
          </div>`:''}
        </div>
...
function renderDashHealthBanner(avgs, totals) {
  ...
  const flags = [];
  const allOpen = jobs.filter(isOpenService);
  const urgent  = allOpen.filter(j => daysBetween(j.poDate,null) >= 21).length;
  const critical = allOpen.filter(j => daysBetween(j.poDate,null) >= 30).length;
  const partsCount = totals['Waiting for Parts']?.c || 0;
  const revisitCount = totals['Revisiting']?.c || 0;

  if (critical > 0) flags.push({ dot:'#dc2626', text: critical + ' critical job' + (critical!==1?'s':'') + ' (30d+)', page:'urgent' });
  else if (urgent > 0) flags.push({ dot:'#d97706', text: urgent + ' overdue job' + (urgent!==1?'s':'') + ' (21d+)', page:'urgent' });
  if (avgs[2] > 21 && partsCount > 0) flags.push({ dot:'#d97706', text: partsCount + ' job' + (partsCount!==1?'s':'') + ' waiting on parts (avg ' + avgs[2] + 'd)', page:'bottleneck' });
  if (revisitCount > 0) flags.push({ dot: avgs[3]>21?'#dc2626':'#d97706', text: revisitCount + ' revisiting (avg ' + avgs[3] + 'd)', page:'bottleneck' });
  ...
  if (hotSites.length) {
    flags.push({ dot:'#7c3aed', text: hotSites[0].name + ' · ' + hotSites[0].count + 'x this year', page:'sites' });
  }

  el.style.display = 'flex';
  if (!flags.length) {
    el.style.cssText = 'display:flex;background:rgba(22,163,74,0.06);border:1px solid rgba(22,163,74,0.2);border-radius:var(--radius-sm);padding:9px 16px;margin-bottom:12px;font-size:12px;gap:8px;align-items:center';
    el.innerHTML = '<span style="font-size:13px">✅</span><span style="font-weight:600;color:var(--green)">All clear — no urgent issues</span>';
    return;
  }

  el.style.cssText = 'display:flex;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:9px 16px;margin-bottom:12px;font-size:12px;gap:0;align-items:center;flex-wrap:wrap;gap:4px 0';
  let html = '<span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:var(--text3);margin-right:12px;flex-shrink:0">This week</span>';
  flags.forEach(function(f, i) {
    html += '<span style="display:inline-flex;align-items:center;gap:5px;margin-right:14px;white-space:nowrap">'
      + '<span style="width:7px;height:7px;border-radius:50%;background:' + f.dot + ';display:inline-block;flex-shrink:0"></span>'
      + '<span style="color:var(--text2)">' + f.text + '</span>'
      + '<span onclick="showPage(\'' + f.page + '\')" style="color:var(--blue);cursor:pointer;font-size:11px;margin-left:2px">↗</span>'
      + '</span>';
    if (i < flags.length - 1) html += '<span style="color:var(--border);margin-right:14px">|</span>';
  });
  el.innerHTML = html;
}
...
function renderSupplierTags() {
  ...
  function makeRow(s) {
    ...
    const safeS = esc(s).replace(/'/g, '&#39;');

    let pillHtml = '';
    if (effectiveState) {
      pillHtml = '<span style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:12px;'
        + 'background:' + sc + '22;color:' + sc + ';border:1px solid ' + sc + '44;flex-shrink:0">'
        + effectiveState + '</span>';
    }

    let optionsHtml = '<option value="">' + (effectiveState ? '✎ change' : '— tag state') + '</option>';
    STATES.forEach(function(st) {
      const sel = (userState || autoState) === st ? ' selected' : '';
      optionsHtml += '<option value="' + st + '"' + sel + '>' + st + '</option>';
    });

    const subText = effectiveState ? (isAuto ? 'Auto-detected' : 'Manually tagged') : 'Not tagged yet';

    return '<div style="display:flex;align-items:center;gap:12px;padding:11px 16px;border-bottom:1px solid var(--border)">'
      + '<div style="flex:1;min-width:0">'
      + '<div style="font-size:13px;font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + safeS + '</div>'
      + '<div style="font-size:11px;color:var(--text3);margin-top:1px">' + subText + '</div>'
      + '</div>'
      + pillHtml
      + '<select onchange="setSupplierTag(\'' + safeS + '\',this.value)" '
  ...
  if (untagged.length) {
    html += '<div class="card" style="margin-bottom:16px;overflow:hidden">'
      + '<div style="padding:14px 20px;background:rgba(220,38,38,0.04);border-bottom:1px solid var(--border)">'
      + '<div style="font-size:13px;font-weight:700;color:var(--text)">Needs tagging '
      + '<span style="font-size:12px;font-weight:400;color:var(--text3)">(' + untagged.length + ' companies)</span></div>'
      + '<div style="font-size:11px;color:var(--text3);margin-top:2px">These companies appear in your jobs but have not been assigned a state yet</div>'
      + '</div>'
  ...
  html += '<div class="card" style="overflow:hidden">'
    ...
    + '<div style="font-size:13px;font-weight:700;color:var(--text)">Tagged companies '
    + '<span style="font-size:12px;font-weight:400;color:var(--text3)">(' + tagged.length + ')</span></div>'
    + '<div style="font-size:11px;color:var(--text3);margin-top:2px">Pre-loaded from your spreadsheet + any manual overrides</div>'
    + '</div>'
