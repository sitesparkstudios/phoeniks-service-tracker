...
const CHART_TICK = { color: '#9ba3af', font: { size: 11, family: 'DM Sans' } };
...
function renderDashboard() {
  ...
  statusChartInst = new Chart(document.getElementById('statusChart'), {
    ...
    options: { responsive: true, maintainAspectRatio: false, cutout: '65%',
      plugins: { legend: { position: 'right', labels: { boxWidth: 10, padding: 12, font: { size: 11, family:'DM Sans' }, color: '#5a6270' } } } }
  });
...
function renderSpendChart() {
  ...
  if (!hasData) {
    ...
    wrap.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text3);font-size:12px">Spend data will appear after importing jobs with values from Odoo</div>';
    return;
  }
...
function renderBottleneck() {
  ...
  dwellChartInst = new Chart(document.getElementById('dwellChart'), {
    type: 'bar',
    data: { labels: ACTIVE_STAGES.map(s => s.replace('Waiting for ','Wait ')), datasets: [{ label:'Avg days', data: avgs, backgroundColor: STAGE_COLORS.slice(0,4), borderRadius: 6, borderWidth: 0 }] },
    options: chartOpts
  });
...
function openJobModal(id) {
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;font-size:13px;margin-bottom:16px">
      <div><div class="text-muted text-sm mb-4">Supplier</div><div style="font-weight:500;color:var(--text)">${esc(j.supplier)}</div></div>
      ...
      <div><div class="text-muted text-sm mb-4">Total Open</div><div class="mono" style="font-weight:600;color:var(--text)">${total !== null ? total + ' days' : '—'}</div></div>
